...
public class Exp3_2 {
    ...
    public static void main(String ag[]) {
        ...
        itemsList.add(new BasicItem("CX002", "Item X0934809 - 3094 093 093409 809 Google Chrome is a fast web browser available at no charge. Before you download, you can check if Chrome supports your operating system and that you have all the other system requirements.", "in_transit", "2024-11-15",
                "2024-11-18"));
        itemsList.add(new BasicItem("CX003", "Item SBSHSHSH", "in_transit", "2024-11-16",
                "2024-11-19"));

        itemsList.add(new BasicItem("CX004", "Item 349sdjuj- The film shows a man drawing a cartoon face on an easel. He draws a bottle of wine and a glass, then takes them off the paper and has a drink. He then gives the cartoon face a drink of wine, and the face breaks into a broad smile. He then draws a hat on the face's head, removes it, and puts it on. Next a cigar appears in the face's mouth, and the man removes it to the face's unhappiness. He then places all of the objects back into the image, and the face's eyes and grin grow wider in appreciation.", "dispatched", "2024-11-20",
                "2024-11-21"));

        LinkedHashMap<String, List<BasicCell>> colsMap = new LinkedHashMap<>();
        ...
        try {
            ...
            for (Field field : itemsList.get(0).getClass().getDeclaredFields()) {
                ...
                colsMap.put(field.getName(), cells);
            }

            // cell processing
            int maxRL = 0;
            List<String> headers = new ArrayList<>();
            var colsMap2 = new LinkedHashMap<String, List<BasicCell>>();
            for (String key : colsMap.keySet()) {
                headers.add(key);
                maxRL += colsMap.get(key).get(0).getLeftPadding() + colsMap.get(key).get(0).getTextWidth()
                        + colsMap.get(key).get(0).getRightPadding();

                colsMap2.put(key, setColSizeAsPerMaxLen(colsMap.get(key)));
            }
            colsMap = colsMap2;
            maxRL += 10;
            ...
            // headers
            {
                ...
                for (int x = 0; x < headers.size(); x++) {
                    ...
                    cellsH.add(new BasicCell(
                            colsMap.get(headers.get(x)).get(0).getLeftPadding(),
                            colsMap.get(headers.get(x)).get(0).getRightPadding(),
                            finalText,
                            colsMap.get(headers.get(x)).get(0).getTextWidth(),
                            CellAlign.center));
                }
                exp3.function4(cellsH, maxRL);
            }

            Map<String, CellAlign> colAlignProps = new HashMap<String, CellAlign>();
            colAlignProps.put("description", CellAlign.left);
            ...
            for (BasicItem citem : itemsList) {
                ...
                for (Field field : citem.getClass().getDeclaredFields()) {
                    ...
                    CellAlign alignprop = (colAlignProps.keySet().contains(field.getName())
                            ? colAlignProps.get(field.getName())
                            : colsMap.get(field.getName()).get(0).getAlign());

                    cellsDH.add(
                            new BasicCell(
                                    colsMap.get(field.getName()).get(0).getLeftPadding(),
                                    colsMap.get(field.getName()).get(0).getRightPadding(),
                                    cfieldValue,
                                    colsMap.get(field.getName()).get(0).getTextWidth(),
                                    alignprop));
                    field.setAccessible(false);
                }

                exp3.function4(cellsDH, maxRL);
    ...
    public void function4(List<BasicCell> cellsH, int rs) {
    ...
    private void handleCellsOverflow(List<BasicCell> cellsH) {
        ...
        for (int x = 0; x < cellsH.size(); x++) {
            ...
            if (!cellsH.get(x).getOverflowInd()) {
                ...
                int ctstopto = overflowedText.length() < cellsH.get(x).getTextWidth()
                        ? overflowedText.length() : cellsH.get(x).getTextWidth();
    ...
    public void printSpaces(int size) {
    ...
    static List<BasicCell> setColSizeAsPerMaxLen(List<BasicCell> cells) {
        Integer[] lens = cells.stream().map(item -> Integer.valueOf(item.getText().length())).toArray(Integer[]::new);
        int ccolmaxval = findMaxLen(lens);
